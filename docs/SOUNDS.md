...
| File | Line | Event |
|------|------|-------|
| `dino_game.dart` | `handleTap()` | Jump sound on tap |
| `dino_game.dart` | `_checkCollisions()` | Death sound on collision |
| `dino_game.dart` | `update()` | Score sound per obstacle passed |
| `dino_game.dart` | `update()` | Milestone sound at each 100pt threshold |
